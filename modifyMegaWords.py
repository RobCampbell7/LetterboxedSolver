...
words = []
with open("megaWordsOld.txt", "r") as dictFile:
...
foundString = {}
for word in words:
    if foundString.get(word, False) == False:
        foundString[word] = True
    
words = sorted(list(foundString.keys()))
with open("megaWords.txt", "w") as dictFile:
    for word in words:
        if allow(word) == True:
            dictFile.write(word + "\n")